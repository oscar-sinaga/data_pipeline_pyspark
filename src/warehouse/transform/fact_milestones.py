...
def transform_fact_milestones(data: pd.DataFrame, table_name: str) -> pd.DataFrame:
    """
    ...
    try:
        ...
        # deduplication based on milestone_nk
        data = data.drop_duplicates(subset='milestone_nk')

        # fill the nan row data
        # Ubah kolom ke datetime, isi NaT dengan 1900-01-01
        data['milestone_at'] = pd.to_datetime(data['milestone_at'], errors='coerce').fillna(pd.Timestamp('2100-01-01'))
        data['milestone_code'] = data['milestone_code'].fillna('N/A')
        data['description'] = data['description'].fillna('N/A')

        # Ubah tipe data
        data['milestone_at'] = data['milestone_at'].astype('int')
